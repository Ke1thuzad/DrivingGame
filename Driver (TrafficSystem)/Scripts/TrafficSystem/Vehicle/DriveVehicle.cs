...
public class DriveVehicle : MonoBehaviour
{
    ...
    public float GetCurrentSpeed()
    {
        ...
        {
            Console.WriteLine(m_VehicleControl.GetSpeed());
            return m_VehicleControl.GetSpeed(); // Предполагаем, что у VehicleControl есть метод GetSpeed, возвращающий скорость в м/с
        }
        return 0f;
    }
    ...
    void Update()
    {
        if (gettingOnCar) GetinCar();
        else if (gettingOnBike) GetinBike();
    }
}
